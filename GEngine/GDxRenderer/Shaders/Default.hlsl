...
float4 PS(VertexOut pin) : SV_Target
{
	// Fetch the material data.
	MaterialData matData = gMaterialData[gMaterialIndex];
	float4 diffuseAlbedo = matData.VectorParams[0];
	float3 fresnelR0 = matData.VectorParams[1].xyz;
	float  roughness = matData.ScalarParams[0];
	...
	//float3 fresnelR0 = float3(1.0f, 1.0f, 1.0f);
