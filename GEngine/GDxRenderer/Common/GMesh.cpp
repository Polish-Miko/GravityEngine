...
GMesh::GMesh(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList, std::vector<MeshData> meshData)
{
	Create(device, cmdList, meshData);
}

void GMesh::Create(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList, std::vector<MeshData> meshData)
{
	UINT vertexOffset = 0;
	UINT indexOffset = 0;
	...
	{
		GSubmesh submesh;
		submesh.IndexCount = (UINT)mdata.Indices32.size();
		...
		indices.insert(indices.end(), std::begin(mdata.Indices32), std::end(mdata.Indices32));

		Submeshes[mdata.SubmeshName] = submesh;

		vertexOffset += (UINT)mdata.Vertices.size();
		indexOffset += (UINT)mdata.Indices32.size();
	}
	...
	GSubmeshGeometry boxSubmesh;
	boxSubmesh.IndexCount = (UINT)box.Indices32.size();
	boxSubmesh.StartIndexLocation = boxIndexOffset;
	boxSubmesh.BaseVertexLocation = boxVertexOffset;

	GSubmeshGeometry gridSubmesh;
	gridSubmesh.IndexCount = (UINT)grid.Indices32.size();
	gridSubmesh.StartIndexLocation = gridIndexOffset;
	gridSubmesh.BaseVertexLocation = gridVertexOffset;

	GSubmeshGeometry sphereSubmesh;
	sphereSubmesh.IndexCount = (UINT)sphere.Indices32.size();
	sphereSubmesh.StartIndexLocation = sphereIndexOffset;
	sphereSubmesh.BaseVertexLocation = sphereVertexOffset;

	GSubmeshGeometry cylinderSubmesh;
	cylinderSubmesh.IndexCount = (UINT)cylinder.Indices32.size();
	cylinderSubmesh.StartIndexLocation = cylinderIndexOffset;
	cylinderSubmesh.BaseVertexLocation = cylinderVertexOffset;

	GSubmeshGeometry quadSubmesh;
